...
export class OffersManagerComponent {
  ...
  displayedColumnsComplete: string[] = [
    'id',
    'title',
    'career',
    'modality',
  ...
  toggle(event: MatSlideToggleChange) {
    event.checked
      ? (this.displayedColumns = this.displayedColumnsResume)
      : (this.displayedColumns = this.displayedColumnsComplete);
  ...
  deleteOffer(id: number): void {
    this.sus = this._OfferService.deleteOfferById(id).subscribe((res) => {
      if (res.Response == 'offer deleted successfully') {
